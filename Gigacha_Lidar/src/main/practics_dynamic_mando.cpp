...
public:
	...
	ImageProjection(): nh("~"){
		...
		// Subscribe할 토픽 설정
		subLidar = nh.subscribe("/scan_2D", 10, &ImageProjection::cloudHandler, this);
...
// 라이다와 GPS만 작동시 진행
	...
	void getPointCloud()
	{
		pointCloudGenerator.getInterestCloud(laserCloudIn, emergencyCloud, {0, 1.8}, {-0.6, 0.6}, {-0.5, 0.5});
		cout << " - emergencyCloud: " << "\033[32m" << emergencyCloud->points.size() << "\033[0m" << endl;
		
		if (emergencyCloud->points.size() > 5)  emergency.data = true;
	...
	void LidarStop(const std_msgs::Bool::ConstPtr& msg)
	{
		if (msg->data)
		{
			emergency.data = false;
			pubEmergencyFromLidar.publish(emergency);
			cout << "LiDAR is finished." << endl;
			cout << "2D 라이다로 뭘할 수 있는데^^" << endl;
			ros::shutdown();			
		} 
	}
};
